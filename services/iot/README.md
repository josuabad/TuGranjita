...
- `ajv-formats`: Para la validación de formatos de fechas ISO 8601.

## Datos simulados JSON

- [lecturas.json](data/lecturas.json): Este archivo incluye 10 registros de diferentes tipos de sensores (Temperatura, Humedad, Presión, Gas y Luminosidad). Se incluye el campo opcional nivel_bateria en la mayoría de las entradas (y se omite intencionalmente en una, la número 8, para demostrar que es opcional), y se asegura de que todos los timestamp estén en formato ISO 8601 (date-time).
- [sensores.json](data/sensores.json): Este archivo describe las características de varios sensores, utilizando los valores permitidos para los campos tipo y estado ("activo," "mantenimiento," "error," "inactivo").
